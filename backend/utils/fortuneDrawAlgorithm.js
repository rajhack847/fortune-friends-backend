...
export const calculateUserWeight = async (userId, fortuneDrawEventId) => {
  try {
...
export const getAllEligibleUsers = async (fortuneDrawEventId) => {
  try {
    ...
    const [users] = await pool.query(
      `SELECT DISTINCT t.user_id
       FROM tickets t
       JOIN payments p ON t.payment_id = p.id
       WHERE t.fortune_draw_event_id = ? AND p.status = 'approved' AND t.status = 'active'`,
      [fortuneDrawEventId]
    );
...
export const selectWinner = async (fortuneDrawEventId) => {
  try {
    // Get all eligible users
    const userIds = await getAllEligibleUsers(fortuneDrawEventId);
    ...
    const userWeights = await Promise.all(
      userIds.map(userId => calculateUserWeight(userId, fortuneDrawEventId))
    );
